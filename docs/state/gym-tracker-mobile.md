...
- **#9 привязан к Internal Testers сам** (ручной `tf-attach` не понадобился), установлен на iPhone.
  ...
  **Device-тест Фазы 2 пройден 25.07:** круг экспорт → share-sheet → «Сохранить в Файлы» → импорт
  ...
  Нативный share-sheet тем же кругом проверен вживую. **Осталось непроверенным:** камера и скан
  этикетки (Ф4a); импорт битого файла (отказ валидатора) на устройстве не гоняли — 18/18 в runtime-тестах.
...
7. [~] **Device-тесты.** ✅ Напоминания (Ф4b) на #7. ✅ Звук rest (Ф1) на #9. ✅ **Импорт + iCloud KV
   (Ф2) на #9 — прошли 25.07.** **Остался:** камера и скан этикетки (Ф4a).
