...
**Фаза 4a (AI-ввод добавок) написана и проверена в web-UI (`3cabcd6`) — ждёт device-теста.**
...
7. [~] **Device-тесты на #7** — напоминания (Ф4b) ✅ прошли (доставка, звук, список), по ним найдены
   и починены 2 дефекта. **Остались непроверенными:** импорт + iCloud KV (Ф2), камера и скан этикетки
   (Ф4a), звук на locked-экране (Ф1 — переделан, проверять уже на #8).
8. [ ] **Собрать билд #8 и отправить в TestFlight** — содержит `2a810e0`: новая программа, звуки 2–3 кГц,
   тап по напоминанию. Команды: `eas build --platform ios --profile production`, затем
   `eas submit --platform ios --latest`. Версия остаётся 1.0.1, номер сборки инкрементится сам.
...
10. [ ] **Опубликовать политику на gh-pages** (`site/privacy.html` → ветка `gh-pages`) — до этого по
   Privacy URL висит старый текст, прямо противоречащий приложению → Guideline 5.1.1, отказ.
...
- [x] **Фаза 4a — AI-ввод добавок** (§2): фото → воркер → предпросмотр → сохранение (`3cabcd6`), ждёт device-теста
- [ ] **Фаза 4b — напоминания** (§2.4/§2.6): агрегированные уведомления, pre-permission, ветвление handler'а
...
- Last commit: `2a810e0` — программа, звуки 2–3 кГц, тап по напоминанию, политика, скилл.
