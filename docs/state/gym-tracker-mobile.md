...
**Фаза 4b (напоминания о добавках) написана и снята в web-UI — не закоммичена, ждёт device-теста.**
...
(3) **Фаза 4b (напоминания) — НЕ начата**, следующая сессия начинает с неё. Разведка уже сделана, см. ниже.

### Фаза 4b — что выяснено перед стартом (25.07, разведка)

- ✅ **§2.6 (ветвление foreground-handler) УЖЕ СДЕЛАНО** в Фазе 1 — `utils/liveActivity.ts:19` глушит звук/баннер
  только при `kind === 'rest_soon' | 'rest_done'`, остальные уведомления показывает штатно. Переделывать НЕ надо
  (прежняя запись в этом файле про «глушит любое» была устаревшей).
- Планирование уведомлений сегодня существует **только** для отдыха (`liveActivity.ts`, DATE-триггеры,
  фиксированные identifier'ы `gymbar-rest-soon|done`). Для добавок планировщика нет вообще.
- Разрешения: `requestWorkoutNotificationPermission()` там же — дёргается при старте Live Activity, без
  экрана-объяснения. Спека §2.4 просит **единый pre-permission экран до системного запроса** (иначе первая
  тренировка заберёт системный prompt раньше добавок).
- В сторе есть флаг `settings.notifications.supplements` (дефолт ВКЛ) и тумблер в Настройках — он сейчас
  ни на что не влияет, планировщика за ним нет.
- Модель: у добавки плоский `schedule: SupplementSlot[]` (morning / pre_workout / evening), **времени нет нигде**.

**Развилка, которую надо решить первым делом:** где живёт время приёма.
(a) **Глобально на слот** в настройках (`morningTime` 08:00 / `eveningTime` 21:00), `pre_workout` — по старту
    тренировки, а не по часам. Минимум UI, ровно покрывает агрегирование §2.4. **Моя рекомендация** для личного
    приложения. (b) Per-supplement `ScheduleEntry[]` со своим временем у каждой добавки (как в спеке §2.2) —
    гибче, но заметно больше UI и миграция модели.

**План 4b:** решить (a)/(b) → `utils/supplementNotifications.ts` (сборка групп по уникальному времени, текст
«08:00 — Витамин D3 2000 МЕ, Омега-3 2 капс», CALENDAR-триггер `hour/minute` + `repeats`, wall-clock без
таймзоны, `data.kind: 'supplement'`) → пересчёт = отменить все свои pending и перепланировать (надёжнее хранения
id) → pre-permission экран → в Настройках время слотов + строка «N активных напоминаний» + баннер, если
разрешения/звук выключены. Бюджет 64 не в риске: при варианте (a) это 2 повторяющихся уведомления.
...
4. [x] **Фаза 4b — напоминания** (§2.4) написана 25.07 (вариант (a)), снята в web-UI, НЕ закоммичена.
5. [ ] **Билд после 4b** — закроет разом device-тесты Фазы 1 (звук), 2 (iCloud/импорт), 4a (камера, скан)
   и 4b (доставка напоминаний, prompt разрешений).
...
- Last commit: `0dd131e` (state). Код: Фаза 4a `3cabcd6`, фикс воркера `a1fe987`. Воркер в проде,
